...
def extract_empresa_from_afip_file(file, filename):
    try:
        if filename.lower().endswith((".xlsx",".xlsm",".xls")):
            file.seek(0)
            top = pd.read_excel(file, sheet_name=0, header=None, nrows=8, dtype=str)
            for val in top.fillna("").astype(str).values.flatten():
                name = extract_empresa_from_text(val)
                if name: return name
    except Exception:
        pass
    base = Path(filename).stem
    base = re.sub(r"CUIT[^0-9]*\d{2}-\d{8}-\d", "", base, flags=re.IGNORECASE)
    base = re.sub(r"\d{11}", "", base)
...
def read_afip(file, filename) -> pd.DataFrame:
    if filename.lower().endswith((".xlsx",".xlsm",".xls")):
        ...
        # probar header=1, sino header=0
        for header in (1, 0):
            try:
                file.seek(0)
                df = pd.read_csv(file, header=header, dtype=str, sep=sep)
                # si todo vino en una sola columna, último intento: sep opuesto
...
def extract_pv(row, c_pv, c_numero):
    ...
    if c_numero:
        ...
        if "-" in s:
            ...
            if left:
                return left.zfill(5)[-5:]
        digits = re.sub(r"\D", "", s)
        if len(digits) >= 5:
            return digits[:5].zfill(5)[-5:]
    return ""
...
st.caption("Subí el AFIP/ARCA y el archivo de **datos** (hoja `datos`) y te devuelvo el Excel para importar.")
    ...
    if afip and datos:
        if st.button("Procesar"):
            try:
                ...
                cuit = extract_cuit_from_afip_file(afip, afip.name)
                empresa = sanitize_name(extract_empresa_from_afip_file(afip, afip.name))
                bio = io.BytesIO()
                with pd.ExcelWriter(bio, engine="openpyxl") as writer:
                    out.to_excel(writer, index=False)
                bio.seek(0)
                fname = f"PARA_IMPORTAR_{cuit}_{empresa}.xlsx"
...
with tab2:
    ...
    datos_files = st.file_uploader("Varios 'datos' por empresa (opcional)", type=["xlsx","xls"], accept_multiple_files=True, key="datos2")
    default_datos = st.file_uploader("Datos DEFAULT (opcional, usado si no hay match por CUIT)", type=["xlsx","xls"], key="datos_default")
    if afips and (datos_files or default_datos):
        if st.button("Procesar lote"):
            try:
                # armar mapa CUIT -> df datos
                datos_map = {}
                for df_file in (datos_files or []):
                    c = extract_cuit_from_datos_file(df_file, df_file.name)
                    if not c: 
                        continue
                    df = read_datos(df_file)
                    datos_map[c] = df
                default_df = read_datos(default_datos) if default_datos else None
                ...
                with zipfile.ZipFile(zipbio, "w", zipfile.ZIP_DEFLATED) as z:
                    for f in afips:
                        try:
                            ...
                            empresa = sanitize_name(extract_empresa_from_afip_file(f, f.name))
                            df_afip = read_afip(f, f.name)
                            datos_df = datos_map.get(cuit, default_df)
                            if datos_df is None:
                                z.writestr(f"ERRORES/{Path(f.name).stem}.txt", "Sin 'datos' para este CUIT y no se subió DEFAULT.")
                                continue
                            ...
                            with pd.ExcelWriter(bio, engine="openpyxl") as writer:
                                out.to_excel(writer, index=False)
                            bio.seek(0)
                            z.writestr(f"{cuit}_{empresa}/PARA_IMPORTAR_{cuit}_{empresa}.xlsx", bio.getvalue())
                        except Exception as ie:
                            z.writestr(f"ERRORES/{Path(f.name).stem}.txt", str(ie))
                zipbio.seek(0)
                st.download_button("⬇️ Descargar ZIP con resultados", data=zipbio.getvalue(), file_name="AFIP2Import_resultados.zip", mime="application/zip")
                st.success("Batch listo.")
            except Exception as e:
                st.error(str(e))
    else:
        st.info("Subí **AFIP(s)** y al menos un **datos** (varios por empresa o un default).")